...
- **所有可编辑变量必须放在脚本开头**，包括但不限于：路径配置、阈值参数、子文件夹选择、API 配置等。
- **禁止使用 `argparse` 或其他命令行参数接收方式**（如 `sys.argv`、click、fire 等），除非用户明确说明需要命令行传参。
- 参数示例：
  ```python
  ...
  BATCH_SIZE = 100
  ```
- 原因：脚本应成为独立可执行的"配置文件+代码"的组合，用户只需修改脚本开头的变量即可运行，无需记忆命令行参数。
- 适用场景：所有非临时性的业务脚本、批处理脚本、数据处理流水线等。
